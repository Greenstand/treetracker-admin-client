...
const verity = {
  ...
  effects: {
    ...
    async loadTreeImages(_payload, state) {
      ...
      if (
        verityState.isLoading ||
        (verityState.treeCount > 0 &&
          verityState.treeImages.length >= verityState.treeCount) ||
        verityState.pageSize * (verityState.currentPage + 1) <=
          verityState.treeImages.length
      ) {
    ...
    clickTree(payload, state) {
      ...
      if (!isShift && !isCmd && !isCtrl) {
        ...
        const treeImagesSelected = state.verity.treeImages
          .slice(
        ...
        if (state.verity.treeImagesSelected.find((el) => el === treeId)) {
          treeImagesSelected = state.verity.treeImagesSelected.filter(function (
            tree,
          ) {
            return tree !== treeId;
          });
