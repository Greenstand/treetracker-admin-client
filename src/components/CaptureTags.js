...
const CaptureTags = (props) => {
  ...
  let handleAddChip = (chip) => {
    context.setTagInput(context.tagInput.concat([chip]));
  };

  let handleDeleteChip = (_chip, index) => {
    const temp = context.tagInput;
    temp.splice(index, 1);
    context.setTagInput(temp);
  };
