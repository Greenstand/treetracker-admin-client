...
const CaptureTable = () => {
  ...
  const { captures, capture, orderBy, order } = context;
  ...
  const loadCaptures = (payload) => {
    context.getCapturesAsync(payload).then(() => {
      scrollRef.current && scrollRef.current.scrollTo(0, 0);
    });
  };

  const toggleDrawer = (id) => {
    context.getCaptureAsync(id);
  ...
  const createSortHandler = (attr) => {
    return () => {
      const order =
        context.orderBy === attr && context.order === 'asc' ? 'desc' : 'asc';
      const orderBy = attr;
